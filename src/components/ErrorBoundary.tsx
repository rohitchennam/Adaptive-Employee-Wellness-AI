    ...
    if (this.state.hasError) {
      ...
      try {
        ...
        if (parsed.error && parsed.operationType) {
          errorMessage = `Database error during ${parsed.operationType}: ${parsed.error}. Please check your permissions.`;
        }
      } catch (e) {
        // Not a JSON error
      }
      ...
      return (
        <div className="min-h-screen bg-bg-deep flex items-center justify-center p-4">
          <div className="glass-card p-10 max-w-md w-full text-center">
            ...
            <p className="text-slate-400 mb-8 font-light">{errorMessage}</p>
            <button 
              onClick={() => window.location.reload()}
              className="glow-button w-full py-4 text-sm tracking-widest uppercase"
            >
              Reload Application
            </button>
          </div>
